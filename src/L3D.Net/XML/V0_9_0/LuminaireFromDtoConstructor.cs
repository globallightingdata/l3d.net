...
namespace L3D.Net.XML.V0_9_0
{
    class LuminaireFromDtoConstructor : ILuminaireFromDtoConstructor
    {
        ...
        {
            options.WithExcludedFromMeasurement(geometryNodeDto.ExcludedFromMeasurement);
            ...
            foreach (var assignment in geometryNodeDto.LightEmittingFaceAssignments)
            {
                if (assignment is AssignmentDto singleAssignement)
                    options.WithLightEmittingSurface(singleAssignement.LightEmittingPartName, singleAssignement.FaceIndex, singleAssignement.GroupIndex);
                else if (assignment is RangeAssignmentDto rangeAssignment)
                    options.WithLightEmittingSurfaces(rangeAssignment.LightEmittingPartName, rangeAssignment.FaceIndexBegin, rangeAssignment.FaceIndexEnd, rangeAssignment.GroupIndex);
                else
                    throw new Exception("Invalid AssignmentDto in GeometryNodeDto: " + assignment?.GetType().FullName);
            }
